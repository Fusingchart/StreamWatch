import { assignWaterway } from './waterways';

export type POIType = 'beach' | 'salmon_habitat' | 'water_intake' | 'park' | 'shellfish' | 'marine';

// Which hydrologically-connected drainage a POI actually sits in. Used to
// stop e.g. a Sammamish River report from showing "downstream impacts" on
// the Snohomish system just because it's geographically west and <100km
// away — they're unconnected watersheds, so pollution can't reach one from
// the other. Skykomish/Wallace/Sultan/Pilchuck are all tributaries that
// feed the Snohomish; Stillaguamish and Sammamish are separate systems
// with no surface connection to it or each other.
type WatershedSystem = 'snohomish_system' | 'stillaguamish_system' | 'sammamish_system';

const WATERWAY_TO_SYSTEM: Record<string, WatershedSystem> = {
  snohomish: 'snohomish_system',
  pilchuck: 'snohomish_system',
  wallace: 'snohomish_system',
  sultan: 'snohomish_system',
  skykomish: 'snohomish_system',
  lake_stevens: 'snohomish_system',
  stillaguamish: 'stillaguamish_system',
  sammamish: 'sammamish_system',
};
...
export function getDownstreamImpacts(latitude: number, longitude: number): DownstreamHit[] {
  const nearestWaterway = assignWaterway(latitude, longitude);
  const system = nearestWaterway ? WATERWAY_TO_SYSTEM[nearestWaterway.id] : undefined;

  return POIS
    // Only POIs in the same connected drainage can actually be downstream
    // of this report — a POI merely being west and <100km away doesn't
    // mean pollution here can reach it.
    .filter((poi) => !system || poi.system === system)
