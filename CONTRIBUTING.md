...
- A factual `description`, including specific stats where possible (visitor counts, species listed, etc.)

### New waterway (`src/data/waterways.ts`)

Add a bounding box that covers the waterway's main stem and major tributaries. The bounds are used to assign sightings, so overly tight bounds will miss reports.
